...
enum{
	Inquiry_Response = 0x00,
	Inquiry_Complete = 0x01,
	AT_Audio_Source_Change = 0x02,
	AT_Mode_Change= 0x03,
	AT_Sampling_Rate_Change = 0x04
};
...
void BT_CommandDecode( void )
{
    ...
    switch(BT_CmdBuffer[0])
    {
        ...
        case DEVICE_STATE:
            ...
            switch(BT_CmdBuffer[1])
            {
				...
				case BT_ACL_CONNECTED:
					BTAPP_EventHandler(BT_EVENT_ACL_CONNECTED, 0, 0 );
				break;
				case BT_ACL_DISCONNECTED:
					BTAPP_EventHandler(BT_EVENT_ACL_DISCONNECTED, 0, 0 );
				break;
				case BT_INQUIRY:
					BTAPP_EventHandler(BT_EVENT_INQUIRY_STARTED, 0, 0 );
				break;
        ...
        case REPORT_BTM_INITIAL_STATUS:
            ...
            break;
		case REPORT_AT_EVENT:
		{
			switch(BT_CmdBuffer[1])
			{
		        case Inquiry_Response:
		            BTAPP_EventHandler(BT_EVENT_INQUIRY_RESPONSE, &BT_CmdBuffer[1], BT_CmdDecodeCmdLength-1 );
		            break;
		        case Inquiry_Complete:
		            BTAPP_EventHandler(BT_EVENT_INQUIRY_COMPLETE, &BT_CmdBuffer[1], BT_CmdDecodeCmdLength-1 );
		            break;
		        case AT_Audio_Source_Change:			
		            BTAPP_EventHandler(BT_EVENT_AT_SOURCE_CHANGE, &BT_CmdBuffer[1], BT_CmdDecodeCmdLength-1 );
		            break;
		        case AT_Mode_Change:
		            BTAPP_EventHandler(BT_EVENT_AT_MODE_CHANGE, &BT_CmdBuffer[1], BT_CmdDecodeCmdLength-1 );			
		            break;
				case AT_Sampling_Rate_Change:
					BTAPP_EventHandler(BT_EVENT_AT_SAMPLING_RATE_CHANGE, &BT_CmdBuffer[1], BT_CmdDecodeCmdLength-1 );			
					break;
			}
		}            
        default:
